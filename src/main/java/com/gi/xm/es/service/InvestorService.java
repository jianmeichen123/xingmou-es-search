...
public class InvestorService extends BaseService {
    ...
    @Value("${ctdn.investor.index}")
    private  String index;

    @Value("${ctdn.investor.type}")
    private  String type;
    ...
    public  Long queryNum(Query query){
        InvestorQuery investorUpQuery = new InvestorQuery();
        investorUpQuery.setKeyword(query.getKeyword());
        SearchRequestBuilder qb = queryList(investorUpQuery);
     ...
     * @return
     */
    public SearchRequestBuilder queryList(InvestorQuery query){
     ...
     * @throws Exception
     */
    public  List<Object> getResponseList (InvestorQuery query,SearchHits shs)  throws Exception{
        List<Object> entityList = new ArrayList<>();
        for (SearchHit it : shs) {
            Map source = it.getSource();
            InvestorQuery p = JSON.parseObject(JSON.toJSONString(source), InvestorQuery.class);
            //获取对应的高亮域
